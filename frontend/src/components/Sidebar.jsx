...
export default function Sidebar({ isOpen, setIsOpen, theme }) {
    ...
    return (
        <aside
            className={`fixed left-0 top-0 h-screen bg-[var(--sidebar-bg,var(--bg-card))] border-r border-[var(--border-color)] transition-all duration-300 z-50 flex flex-col ${isOpen ? 'w-60' : 'w-20'}`}
        >
                        ...
                        <div key={item.name} className="relative">
                            {item.name === "Financial Architecture" && (
                                <div className="mt-8 mb-3">
                                    {isOpen && <p className="px-3 text-[10px] font-bold uppercase tracking-[0.15em] text-[var(--text-muted)] opacity-50">Operations</p>}
                                </div>
                            )}
                            <button
